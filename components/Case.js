...
export default function Case({ data, isPlayerCase, phase, onSelect, onOpen, mobile }) {
  ...
  return (
    // On mobile: fill the grid cell (w-full). On desktop: fixed width set by parent column.
    <div className={mobile ? 'w-full aspect-[4/5]' : 'w-full aspect-[4/5]'}>
  ...
  return (
    <motion.button
      ...
      className={[
        ...
        interactive
          ? 'cursor-pointer hover:border-sky-400/60 hover:shadow-[0_0_20px_rgba(57,189,248,0.4),inset_0_1px_0_rgba(255,255,255,0.6)]'
          : 'cursor-default',
    ...
    >
      ...
      {interactive && (
        <div className="pointer-events-none absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-xl">
          <div className="absolute -inset-y-2 -left-1/2 w-1/3 bg-white/18 blur-md animate-shine" />
        </div>
      )}
      ...
      {/* "Yours" badge */}
      {isPlayerCase && (
        <span className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[7px] uppercase tracking-[0.2em] font-black text-white bg-black/40 px-1.5 py-0.5 rounded whitespace-nowrap">
          Yours
        </span>
      )}
    </motion.button>
